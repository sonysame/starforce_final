...
def StmtAnalysis(arg):
      ...
      if sub_target in LeftHandSideList and re.search(r"\[.*\]",re.findall(r"=([\W\w]*);",LeftHandSideList[sub_target][0])[0]) is None and find_function(re.findall(r"=([\W\w]*);",LeftHandSideList[sub_target][0])[0]):
         ...
         for i in FunctionList:
            if i in ReadLine:
               ...
               break
         if FunctionExcludeFlag is False and LeftHandSideList[sub_target][0]:
            if arg[0] != sub_target:
   ...
   if ArgNum == 3:
      ...
      if sub_target in LeftHandSideList and re.search(r"\[.*\]",re.findall(r"=([\W\w]*);",LeftHandSideList[sub_target][0])[0]) is None and find_function(re.findall(r"=([\W\w]*);",LeftHandSideList[sub_target][0])[0]):
         ...
         if FunctionExcludeFlag is False:
            if arg[0] != sub_target:
               PropagationFlag = True
               ReadLine = re.sub(re.escape(sub_target), '('+re.findall(r"=([\W\w]*);",LeftHandSideList[sub_target][0])[0]+')', ReadLine)
...
def Compile_win(SourceCodeName, SourceCode, Output):
   Compile = "clang " +SourceCodeName+ "." + SourceCode.split(".")[1] + " -o "+ Output
   os.system(Compile)
...
def Clear_win(SourceCodeName):
   ch=input("Do you want to remove all the previous files? (Y/N) ")
   if(ch=='Y' or ch=='y'):
      Clear = "del " + SourceCodeName+"*"
      os.system(Clear)
...
def Execute_win(ProgramName, Log): 
   Execute = ProgramName+".exe"
   if Log:
      Execute += " > " + ProgramName + "[Log].txt"
   os.system(Execute)
...
def Optimization(SourceCode):
   ...
   if(Round==1):   
      ...
      OptimizedSourceCode.write("#pragma clang diagnostic ignored \"-Weverything\"\n")
   ...
   if(Round==1):
      ...
      #Compile_win(LabelAppendedCodeName, SourceCode, LabelAppendedCodeName+".exe")
      #Execute_win(LabelAppendedCodeName, True)
      try:
   ...
   #Compile_win(OptimizedCodeName, SourceCode, OptimizedCodeName+"_clang.exe")
   #Execute_win(OptimizedCodeName+"_clang", False)
...
if __name__=="__main__":
   ...
   if OnlyControlFlow:
      ...
      #Compile_win(FinalCodeName.split(".")[0], FinalCodeName, FinalCodeName.split(".")[0]+"_clang.exe")
      #Execute_win(FinalCodeName.split(".")[0]+"_clang", False)
